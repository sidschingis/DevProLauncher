...
function c4779823.initial_effect(c)
	...
	e1:SetCountLimit(1)
	e1:SetCost(c4779823.remcost)
	e1:SetTarget(c4779823.remtg)
	e1:SetOperation(c4779823.remop)
	c:RegisterEffect(e1)
	--todeck
...
function c4779823.remcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckLPCost(tp,1000) end
	Duel.PayLPCost(tp,1000)
end
function c4779823.remfilter(c)
	return c:IsAbleToRemove()
end
function c4779823.remtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingTarget(c4779823.remfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectTarget(tp,c4779823.remfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,g,1,0,0)
end
function c4779823.remop(e,tp,eg,ep,ev,re,r,rp)
